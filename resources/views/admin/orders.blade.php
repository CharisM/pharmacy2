...
<div class="admin-shell">
    ...
    <main class="admin-main">
        ...
        <div class="status-tabs">
            <a href="{{ route('admin.orders') }}" class="status-tab {{ !request('status') ? 'active' : '' }}">
                All <span class="tab-count">{{ $counts['all'] }}</span>
            </a>
            @foreach($statuses as $s)
            <a href="{{ route('admin.orders', ['status' => $s] + request()->except('status', 'page')) }}"
               class="status-tab status-tab-{{ strtolower($s) }} {{ request('status') == $s ? 'active active-'.$s : '' }}">
        ...
        <div class="filters-bar">
            <form method="GET" action="{{ route('admin.orders') }}" class="filters-form">
                ...
                <div class="search-wrap">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name or order ID…" class="search-input" />
                </div>
                <button type="submit" class="btn-filter">Search</button>
                @if(request()->hasAny(['search']))
                    <a href="{{ route('admin.orders', request()->only('status')) }}" class="btn-clear">Clear</a>
                @endif
            </form>
        </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action-btns">
                                    ...
                                    <button class="btn-status" onclick="openStatusModal({{ $order->id }}, '{{ $order->status }}')">
...
<div id="orderViewModal" class="modal-overlay" style="display:none;" onclick="if(event.target===this)this.style.display='none'">
    <div class="order-modal-box">
        ...
        <div class="ovm-body">
            ...
            <div class="ovm-section">
                ...
                <div class="ovm-grid-2">
                    ...
                    <div class="ovm-item full"><span>Delivery Address</span><strong id="ovm-address"></strong></div>
                </div>
            </div>
            ...
            <div class="ovm-section">
                ...
                <div class="ovm-grid-3">
                    ...
                    <div class="ovm-item full"><span>Notes</span><strong id="ovm-notes"></strong></div>
                </div>
            </div>
            ...
            <div class="ovm-section">
                ...
                <table class="items-table">
                    ...
                    <tfoot>
                        <tr><td colspan="3" class="total-label">Total</td><td class="total-val" id="ovm-total"></td></tr>
                    </tfoot>
                </table>
            </div>
        </div>
        ...
        <div class="ovm-footer">
            ...
            <button class="btn-status" id="ovm-update-btn" onclick="">
...
<style>
    * { box-sizing: border-box; }
    ...
    .btn-status { display:inline-flex;align-items:center;gap:5px;padding:6px 12px;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;border:none;border-radius:7px;font-weight:600;font-size:0.8rem;cursor:pointer;transition:all 0.2s;white-space:nowrap; }
    .btn-status:hover { transform:translateY(-1px);box-shadow:0 4px 12px rgba(99,102,241,0.4); }
    ...
    .status-modal-box { background:#fff;border-radius:16px;padding:0;width:100%;max-width:400px;box-shadow:0 20px 60px rgba(0,0,0,0.2);margin:auto;overflow:hidden; }
    ...
    @media (max-width: 768px) { .admin-main { margin-left:0;padding:16px; } .ovm-grid-2,.ovm-grid-3 { grid-template-columns:1fr; } }
</style>
